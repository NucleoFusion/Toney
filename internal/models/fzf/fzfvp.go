...
func ItemLineStyle(w int) lipgloss.Style {
	return lipgloss.NewStyle().Width(w).Foreground(colors.ColorPalette().Text)
}

func SelectedItemStyle(w int) lipgloss.Style {
	return ItemLineStyle(w).Background(colors.ColorPalette().MenuSelectedBg).Foreground(colors.ColorPalette().MenuSelectedText)
}
